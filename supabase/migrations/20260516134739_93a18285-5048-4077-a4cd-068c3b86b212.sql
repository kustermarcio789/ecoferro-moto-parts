...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    SELECT id, name, sku, COALESCE(wholesale_price, price) AS price 
    INTO v_product 
    ...
    INSERT INTO public.order_items (
        order_id, product_id, product_name, sku, quantity, unit_price, total, priority
    ) VALUES (
        v_order_id, v_product.id, v_product.name, v_product.sku, v_qty, v_unit_price, v_unit_price * v_qty, v_priority
    );
